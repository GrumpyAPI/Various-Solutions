        ...
        static void Main(string[] args)
        {
            ...
            foreach (var command in commands)
            {
                ...
                if (CheckForEscaping(currentXpos, currentYpos, lair))
                {
                    ...
                    if (currentXpos == lair.GetLength(1))
                    {
                        currentXpos--;
                    }
                    if (currentYpos < 0)
                    ...
                    if (currentYpos == lair.GetLength(0))
                    {
                        currentYpos--;
                    }
            ...
            if (dead)
            {
                Console.WriteLine($"dead: {currentYpos} {currentXpos}");
            }
            else
            {
                Console.WriteLine($"won: {currentYpos} {currentXpos}");
            }
        }
        ...
        static void BunniesSpread(char[,] matrix)
        {
            for (int i = 0; i < matrix.GetLength(0); i++)
            {
                for (int j = 0; j < matrix.GetLength(1); j++)
                {
                    if (matrix[i, j] == 'B')
                    {
                        if (!CheckForEscaping(j - 1, i, matrix) && matrix[i, j - 1] == '.')
                        {
                            matrix[i, j - 1] = 'N';
                        }
                        if (!CheckForEscaping(j + 1, i, matrix) && matrix[i, j + 1] == '.')
                        ...
                        if (!CheckForEscaping(j, i + 1, matrix) && matrix[i + 1, j] == '.')
                        {
                            matrix[i + 1, j] = 'N';
                        }
                        if (!CheckForEscaping(j, i - 1, matrix) && matrix[i - 1, j] == '.')
